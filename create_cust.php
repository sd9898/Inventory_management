...
if(empty($name) || empty($id) || empty($email) || empty($password) || empty($repassword) ||
	...
	$sql = "SELECT custid FROM cust WHERE email_id = '$email' LIMIT 1" ;
	...
	if($count_email > 0){
        ...
        if (mysqli_query($conn, $sql)) {
			...
			echo "register_fail try with some onter user". mysqli_error($connn);
          }
